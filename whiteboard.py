...
while cv2.waitKey(1) != 27:
    _, img = cap.read()
    flag, display_img = cap.read()

    if not _ or not flag:
        break
        ...
        for hand_landmark in res.multi_hand_landmarks:
            ...
            for id, landmark in enumerate(landmarks):
                ...
                if id == 8:
                    ...
                    if abs(index_y - thumb_y) < 60 and abs(index_x - thumb_x) < 60:
                        if(prev_x == 0 and prev_y == 0):
                            ...
                            cv2.line(output_img, (prev_x, prev_y), (x,y), (0,255,0), 5, cv2.LINE_AA)
                    prev_x = x
                    prev_y = y
                ...
                if id == 12:
                    ...
                    middle_x = x
                    middle_y = y
                    if abs(index_y - middle_y) < 60 and abs(index_x - middle_x) < 60:
                        cv2.circle(output_img, (index_x, index_y), 10, (255,255,255), 20)
                        
    cv2.imshow("WhiteBoard", img)
    cv2.imshow("Input", display_img)
    cv2.imshow("WhiteBoard", output_img)